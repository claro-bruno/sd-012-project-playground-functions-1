// Desafio 10
function techList(arrayTechName, name) {
  if (arrayTechName.length === 0) {
    return 'Vazio!';
  }
  let lista = [];
  let arrayTechNameSort = arrayTechName.sort();
   for (let key in arrayTechNameSort) {
      lista.push(
      {
        tech: arrayTechName[key],
        name: name,
      };
      )
   }
